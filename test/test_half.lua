...
function torchtest.easy()
  ...
  local expected = torch.LongStorage{6,1}
  for i=1,stride:size() do
     mytester:assert(stride[i] == expected[i], "stride is wrong")
  end

  x=x:t()
  mytester:assert(not x:isContiguous(), 'x transpose should not be contiguous')
  x=x:transpose(1,2)
  mytester:assert(x:isContiguous(), 'x should be contiguous after 2 transpose')

  local y=torch.HalfTensor()
  y:resizeAs(x:t()):copy(x:t())
  mytester:assert(x:isContiguous(), 'after resize, x should be contiguous')
